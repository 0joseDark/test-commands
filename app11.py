...
def replay_commands():
    try:
        with open(command_file, "r") as file:
            for line in file:
                # Log para debug
                print(f"Reproduzindo: {line.strip()}")
                if "|" in line:
                    _, _, comando = line.rsplit('|', 2)
                    tecla = comando.strip().split(": ")[-1]
                    if target_window:
                        target_window.activate()
                        pyautogui.press(tecla)
                        # Aguardar um pouco para simular a execução dos comandos
                        time.sleep(1)
...
root = tk.Tk()
root.title("Controlador de Janela com Setas")

# Assegura que o arquivo existe
ensure_file_exists()

# Criando os botões de direção
btn_up = tk.Button(root, text="↑", width=10, height=2, command=lambda: press_and_log("up"))
btn_down = tk.Button(root, text="↓", width=10, height=2, command=lambda: press_and_log("down"))
btn_left = tk.Button(root, text="←", width=10, height=2, command=lambda: press_and_log("left"))
btn_right = tk.Button(root, text="→", width=10, height=2, command=lambda: press_and_log("right"))
