...
class ClickRecorderReproducer:
    ...
    def load_images(self):
        ...
        for direction in directions:
            image = Image.open(f'/mnt/data/teclas-movimento-{direction}.jpg')
    ...
    def create_movement_buttons(self):
        ...
        functions = [self.move_up, self.move_down, self.move_left, self.move_right, 
                     self.move_up_left, self.move_up_right, self.move_down_left, self.move_down_right]
        
        for direction, func in zip(directions, functions):
            button = tk.Button(self.root, image=self.images[direction], command=func)
            button.pack(pady=5)
    ...
    def update_status(self, message):
        # Atualiza o status atual (neste caso, imprime no console)
        print(message)
    ...
    def on_press(self, key):
        # Manipulador de eventos de pressão de teclas
        if self.recording and not self.paused:
