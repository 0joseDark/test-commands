...
def select_window():
    ...
    if window_title:
        ...
        lbl_window_status.config(text="Digite o título da janela")
...
# Entrada e botão para selecionar a janela alvo
entry_window_title = tk.Entry(root)
entry_window_title.grid(row=3, column=0, pady=10)
btn_select_window = tk.Button(root, text="Selecionar Janela", command=select_window)
btn_select_window.grid(row=3, column=1, pady=10)
lbl_window_status = tk.Label(root, text="Nenhuma janela selecionada")
lbl_window_status.grid(row=3, column=2, pady=10)
